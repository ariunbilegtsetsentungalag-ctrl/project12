...
exports.postSignup = async (req, res) => {
  try {
    ...
    const role = accountType === 'business_owner' ? 'business_owner' : 'customer';
    ...
    const user = new User({
      ...
      password,
      role,
      // Initialize bundle for business owners
      businessBundle: role === 'business_owner' ? {
        tier: 'none',
        productsAllowed: 0,
        productsUsed: 0
      } : undefined
    ...
    // Redirect business owners to bundle purchase page
    if (role === 'business_owner') {
      req.flash('success', 'Welcome! Please purchase a product bundle to start selling.');
      req.session.save(() => {
